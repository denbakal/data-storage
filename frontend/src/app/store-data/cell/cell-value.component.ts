...
export class CellValueComponent implements OnInit {
  ...
  initData() {
    for (let i = 0; i < CellValueComponent.DEFAULT_LANE_COUNT; i++) {
      this.data.push({});
    }

    this.baseFieldService
      .getFieldsByBaseTableId(CellValueComponent.BASE_TABLE_ID)
      .subscribe((fields) => {
    ...
    this.baseFieldService
      .getFieldValuesByBaseTableId(CellValueComponent.BASE_TABLE_ID, 'CELL')
      .subscribe((data) => {
  ...
  saveData() {
    ...
    this.baseFieldService.storeData(array, 'CELL')
      .subscribe((status) => {
        if (status === 200) {
          this.isSaving = false;
        }
  ...
  removeData() {
    this.baseFieldService.removeData(CellValueComponent.BASE_TABLE_ID, 'CELL')
      .subscribe((status) => {
        if (status === 200) {
          this.initData();
        }
